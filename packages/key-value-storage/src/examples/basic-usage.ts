...
async function localStorageExample() {
  const storage = new KeyValueStorage(new LocalKeyValueStorageProvider());
...
async function memoryStorageExample() {
  const storage = new KeyValueStorage(new MemoryKeyValueStorageProvider());
...
async function remoteStorageExample() {
  const storage = new KeyValueStorage(
    new RemoteKeyValueStorageProvider("https://api.example.com")
  );
...
async function cascadingStorageExample() {
  const storage = new KeyValueStorage(
    new CascadingKeyValueStorageProvider([
      {
        name: "memory",
        storageProvider: new MemoryKeyValueStorageProvider(),
        maxSize: 1000,
        ttl: 5 * 60 * 1000, // 5分钟过期
      },
      {
        name: "local",
        storageProvider: new LocalKeyValueStorageProvider(),
      },
      {
        name: "remote",
        storageProvider: new RemoteKeyValueStorageProvider(
