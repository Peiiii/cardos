...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  const helloCommand = context.commandRegistry.registerCommand(
    "hello",
    (name?: string) => {
  ...
  // 存储一些状态 - 使用工作区存储
  await context.storage.workspace.set(
    "lastActivated",
    new Date().toISOString()
  );

  // 使用全局存储
  await context.storage.global.set(
    "activationCount",
  ...
  // 注意：不需要添加命名空间前缀，代理会自动处理
  const serviceRegistration = context.serviceRegistry.registerService(
    "greetingService",
    greetingService
  );
  ...
  // 检查是否可以访问全局服务和命令
  // 访问另一个扩展的服务（通过完整ID）
  const otherService = context.serviceRegistry.getService(
    "other.extension.someService"
  );
  if (otherService) {
  ...
  // 记录存储状态
  const activationCount = await context.storage.global.get<number>(
    "activationCount"
  );
